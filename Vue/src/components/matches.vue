<template>
    <div>
        <navibar id="navibar" style="background:white" v-bind:isLogged="isLogged"></navibar>
        <login></login>
        
        <div style="text-align:center;" id="container">
            <h2 style="padding-bottom:15px;">最新赛况</h2>
            <img src="https://imgur.com/08JovDj.jpg" alt="对垒表" class="responsive" id="myImg" v-on:click="imgOpen">
            <p style="font-color:gray">点击图片放大</p>
        </div>

        <div id="myModal" class="modal">

            <!-- The Close Button -->
            <span class="close" v-on:click="close">&times;</span>

            <!-- Modal Content (The Image) -->
            <img class="modal-content" id="img01">

            <!-- Modal Caption (Image Text) -->
            <div id="caption"></div>
        </div>
        <footer id="footbar"><foot></foot></footer>

    </div>

</template>

<script>
    import foot from './foot.vue'
    import login from './login.vue'
    import navibar from './navigation.vue'

    export default {
        name: 'matches',
        components:{foot,navibar,login},
        data(){
            return{
               
            }
        },
        computed : {
            isLogged : function(){return this.$store.getters.isLoggedIn},
        },
        methods:{
            close(){
                // console.log("clicked");
                var modal = document.getElementById("myModal");
                var navibar = document.getElementById("navibar");
                modal.style.display = "none";
                navibar.style.display = "";
            },
            imgOpen(){
                 var modal = document.getElementById("myModal");
                var navibar = document.getElementById("navibar");
                // Get the image and insert it inside the modal - use its "alt" text as a caption
                var img = document.getElementById("myImg");
                var modalImg = document.getElementById("img01");
                var captionText = document.getElementById("caption");
                modal.style.display = "block";
                modalImg.src = img.src;
                captionText.innerHTML = img.alt;
                navibar.style.display="none";
            }
        },
        mounted(){
            window.scrollTo(0,0);
        }
    }
    
</script>

<style scoped>
  .responsive{
      padding: (0.3,0.3,0.3,0.1);
      width: 80%;
      height: auto;
      border-radius: 5px;
      cursor: pointer;
      transition: 0.3s;
  }
  .responsive:hover{
      opacity: 0.7;
  }

  /* The Modal (background) */
.modal {
  display: none; /* Hidden by default */
  position: fixed; /* Stay in place */
  z-index: 1; /* Sit on top */
  padding-top: 80px; /* Location of the box */
  left: 0;
  top: 0;
  width: 100%; /* Full width */
  height: 100%; /* Full height */
  overflow: auto; /* Enable scroll if needed */
  background-color: rgb(0,0,0); /* Fallback color */
  background-color: rgba(0,0,0,0.9); /* Black w/ opacity */
}

/* Modal Content (Image) */
.modal-content {
  margin: auto;
  display: block;
  width: 100%;
  max-width: 90%;
}

/* Caption of Modal Image (Image Text) - Same Width as the Image */
#caption {
  margin: auto;
  display: block;
  width: 80%;
  max-width: 700px;
  text-align: center;
  color: #ccc;
  padding: 10px 0;
  height: 150px;
}

/* Add Animation - Zoom in the Modal */
.modal-content, #caption { 
  animation-name: zoom;
  animation-duration: 0.6s;
}

@keyframes zoom {
  from {transform:scale(0)} 
  to {transform:scale(1)}
}

/* The Close Button */
.close {
  position: fixed;
  bottom: 15px;
  right: 35px;
  color: #f1f1f1;
  font-size: 40px;
  font-weight: bold;
  transition: 0.3s;
}

.close:hover,
.close:focus {
  color: #bbb;
  text-decoration: none;
  cursor: pointer;
}

/* 100% Image Width on Smaller Screens */
@media only screen and (max-width: 700px){
  .modal-content {
    width: 100%;
  }
}
</style>
